...
class slave(Robot):
    ...
    def positionCalculator(self, pBest, gBest):
        ...
        self.gCalc = (m1 - g_m2) / (1 + (m1*g_m2)) % 360
        self.pCalc = (m1 - p_m2)/ (1 + (m1*p_m2))
        self.G_angle = round(np.arctan(self.gCalc),2)
        self.P_angle = round(np.arctan(self.pCalc),2)
        
        
        
        #Angle of change
        self.aoc = (round(self.gCalc))
        print("The angle of between projected pos and gBest is", self.G_angle, "gCalc =", self.gCalc)
    ...
    def run(self):
        ...
        while True:
            if self.mode == self.Mode.MOVE:
                ...
                self.newProjected = self.gps.getValues() + (self.velocity*10)
                m1 =  (self.projectedPos[1] - self.currentPos[1])/(self.projectedPos[0] - self.currentPos[0])
                ...
                if self.gCalc-5 <= calc <= self.gCalc+5:
                    self.mode = self.Mode.MOVE
                    print("Change Mode Move", self.name)
                else:
                    self.motors[0].setVelocity(self.max_speed/2)
